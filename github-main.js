 ...
 * GITHUB_TOKEN from your .env and prints the authenticated identity. The token itself is
 * never printed.
 *
 * Usage:
 *   node github-main.js
 *
 * Requires GITHUB_TOKEN in your .env (a PAT — classic with `repo` scope, or fine-grained
 * with Actions read/write on the target repo). See .env.example.
 */
...
async function main() {
  try {
    ...
    console.error('[github-main] Check that GITHUB_TOKEN is set in your .env and is valid (see .env.example).');
    process.exitCode = 1;
  }
}
